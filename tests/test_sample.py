...
def test_wrong_qmc_dimension(x0_batch_2d, x1_batch_2d, f_batch_2d):
    """Test error raised if dimension of user-provided QMC engine is wrong."""
    engine = Sobol(d=2, scramble=True, seed=42)
    with pytest.raises(ValueError):
        sample(
            x0_batch_2d, x1_batch_2d, *f_batch_2d, qmc=True, qmc_engine=engine
        )


def test_wrong_qmc(x0_batch_2d, x1_batch_2d, f_batch_2d):
...
def test_non_power2_sobol_warning():
    ...
    with pytest.warns(UserWarning):
        sample(15, 25, 0.5, 0.5, N_samples=20, qmc=True)


def test_qmc_flag_engine_warning(x0_batch_2d, x1_batch_2d, f_batch_2d):
    """Test warning raised if user-provided qmc engine while qmc flag False"""
    engine = Sobol(d=3, scramble=True, seed=42)
    with pytest.warns(UserWarning):
        sample(
            x0_batch_2d, x1_batch_2d, *f_batch_2d, qmc=False, qmc_engine=engine
        )


def test_qmc_seed_warning(x0_batch_2d, x1_batch_2d, f_batch_2d):
    """Test warning raised if user-provided qmc engine while seed also given"""
    engine = Sobol(d=3, scramble=True, seed=42)
    with pytest.warns(UserWarning):
        sample(
            x0_batch_2d, x1_batch_2d, *f_batch_2d, qmc=True, qmc_engine=engine, seed=42
        )
...
def test_same_int_seed_qmc():
    """Test same RNG seed (specified as integer) produces same results (QMC)."""
    x1 = sample(10, 20, 0.1, 0.3, seed=42, qmc=True)
    x2 = sample(10, 20, 0.1, 0.3, seed=42, qmc=True)
    assert x1==x2
    x1 = sample(10, 20, 0.1, 0.3, N_samples=8, seed=42, qmc=True)
    x2 = sample(10, 20, 0.1, 0.3, N_samples=8, seed=42, qmc=True)
    assert np.all(x1==x2)
...
def test_same_rng_seed_qmc():
    """Test same RNG seed (specified as np RNG) produces same results (QMC)."""
    x1 = sample(10, 20, 0.1, 0.3, seed=np.random.default_rng(42), qmc=True)
    x2 = sample(10, 20, 0.1, 0.3, seed=np.random.default_rng(42), qmc=True)
    assert x1==x2
    x1 = sample(10, 20, 0.1, 0.3, N_samples=8, seed=np.random.default_rng(42), qmc=True)
    x2 = sample(10, 20, 0.1, 0.3, N_samples=8, seed=np.random.default_rng(42), qmc=True)
    assert np.all(x1==x2)


def test_qmc_engine_reset(x0_batch_2d, x1_batch_2d, f_batch_2d):
    """Test same results after resetting QMC engine."""
    engine = Sobol(d=3, scramble=True, seed=42)
    x1 = sample(x0_batch_2d, x1_batch_2d, *f_batch_2d, N_samples=16, qmc=True, qmc_engine=engine)
    engine.reset()
    x2 = sample(x0_batch_2d, x1_batch_2d, *f_batch_2d, N_samples=16, qmc=True, qmc_engine=engine)
    assert np.all(x1==x2)
...
def test_shape_single_sample_single_1D_qmc():
    """Drawing single sample from single 1D cell -> float."""
    x = sample(10, 20, 0.1, 0.3, seed=42, qmc=True)
    assert isinstance(x, float)
...
def test_shape_multi_sample_single_1D_qmc():
    """Drawing multi samples from single 1D cell -> 1D array (N_samples,)."""
    x = sample(10, 20, 0.1, 0.3, N_samples=16, seed=42, qmc=True)
    assert x.shape == (16,)
...
def test_shape_single_sample_single_kD_qmc(x0_single_3d, x1_single_3d, f_single_3d):
    """Drawing single sample from single kD cell -> 1D array (k,)."""
    x = sample(x0_single_3d, x1_single_3d, *f_single_3d, seed=42, qmc=True)
    assert x.shape == (3,)
...
def test_shape_multi_sample_single_kD_qmc(x0_single_3d, x1_single_3d, f_single_3d):
    """Drawing multi samples from single kD cell -> 2D array (N_samples, k)."""
    x = sample(x0_single_3d, x1_single_3d, *f_single_3d, N_samples=16, seed=42, qmc=True)
    assert x.shape == (16, 3)
...
def test_shape_single_sample_multi_kD_qmc(x0_batch_2d, x1_batch_2d, f_batch_2d):
    """Drawing single sample from multi kD cells -> 1D array (k,)."""
    x = sample(x0_batch_2d, x1_batch_2d, *f_batch_2d, seed=42, qmc=True)
    assert x.shape == (2,)
...
def test_shape_single_sample_multi_1D_qmc(x0_batch_1d, x1_batch_1d, f_batch_1d):
    """Drawing single sample from multi 1D cells -> float."""
    x = sample(x0_batch_1d, x1_batch_1d, *f_batch_1d, seed=42, qmc=True)
    assert isinstance(x, float)
...
def test_shape_multi_sample_multi_kD_qmc(x0_batch_2d, x1_batch_2d, f_batch_2d):
    """Drawing multi samples from multi kD cells -> 2D array (N_samples, k)."""
    x = sample(x0_batch_2d, x1_batch_2d, *f_batch_2d, N_samples=16, seed=42, qmc=True)
    assert x.shape == (16, 2)
...
def test_shape_multi_sample_multi_1D_qmc(x0_batch_1d, x1_batch_1d, f_batch_1d):
    """Drawing multi samples from multi 1D cells -> 1D array (N_samples,)."""
    x = sample(x0_batch_1d, x1_batch_1d, *f_batch_1d, N_samples=16, seed=42, qmc=True)
    assert x.shape == (16,)
...
def test_1D_uniform_single_qmc():
    """Test that single uniform QMC sample is in appropriate range."""    
    x = sample(15, 25, 0.5, 0.5, qmc=True)
    assert (x < 25) and (x > 15)


def test_1D_uniform():
    """Test that batch of uniform samples gives flat histogram."""    
    x = sample(15, 25, 0.5, 0.5, N_samples=2**20)
    p = np.histogram(x, np.linspace(15, 25, 11), density=True)[0]
    assert np.all(np.round(p, decimals=1) == 0.1)


def test_1D_uniform_qmc():
    """Test that batch of uniform QMC samples gives flat histogram."""    
    x = sample(15, 25, 0.5, 0.5, N_samples=2**20, qmc=True)
    p = np.histogram(x, np.linspace(15, 25, 11), density=True)[0]
    assert np.all(np.round(p, decimals=1) == 0.1)    
...
def test_1D_GMM():
    ...
    # draw samples
    x = sample(x0, x1, f0, f1, N_samples=2**20, seed=42)
...
def test_1D_GMM_qmc():
    """Test QMC samples from 1D 2-comp GMM have correct means and widths."""
    # true params
    mua_true = -10.0
    mub_true = 10.0
    siga_true = 1.1
    ...
    # draw samples
    x = sample(x0, x1, f0, f1, N_samples=2**20, seed=42, qmc=True)
...
def test_1D_GMM_qmc_halton():
    """Test 1D GMM with user-provided Halton engine."""
    # true params
    mua_true = -10.0
    mub_true = 10.0
    siga_true = 1.1
    ...
    # draw samples
    engine = Halton(d=2)
    x = sample(x0, x1, f0, f1, N_samples=2**20, qmc=True, qmc_engine=engine)
...
def test_2D_GMM():
    ...
    # draw samples
    x = sample(x0, x1, f00, f01, f10, f11, N_samples=2**21, seed=42)
...
def test_2D_GMM_qmc():
    """Test QMC samples from 2D 2-component GMM have correct means and covs."""
    # true params
    mua_true = np.array([-5, -5])
    mub_true = np.array([5, 5])
    cova_true = np.array([
    ...
    # draw samples
    x = sample(x0, x1, f00, f01, f10, f11, N_samples=2**21, seed=42, qmc=True)
...
def test_2D_GMM_qmc_halton():
    """2D GMM test with Halton engine"""
    # true params
    mua_true = np.array([-5, -5])
    mub_true = np.array([5, 5])
    cova_true = np.array([
    ...
    # draw samples
    engine = Halton(d=3)
    x = sample(x0, x1, f00, f01, f10, f11, N_samples=2**21, qmc=True, qmc_engine=engine)
